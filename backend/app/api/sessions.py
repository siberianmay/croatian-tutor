...
async def start_session(
    ...
    """
    session = await crud.create(current_user.id, request)
...
async def list_sessions(
    ...
    total = await crud.count(current_user.id, exercise_type=exercise_type)
...
async def get_active_session(
    ...
    """
    session = await crud.get_active(current_user.id, exercise_type)
    if session:
        return SessionResponse.model_validate(session)
    return None
...
async def get_session(
    ...
    session = await crud.get(session_id, current_user.id)
    if not session:
        raise HTTPException(
...
async def end_session(
    ...
    """
    session = await crud.end_session(session_id, current_user.id, request)
    if not session:
        raise HTTPException(
